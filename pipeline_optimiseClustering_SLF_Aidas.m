...
%keep_nouns = 800;
%keep_feats = 1000;
%[n_inds f_inds] = func_get_reduced_indices(wiki,keep_nouns,keep_feats);
%wiki = func_slice_wiki(wiki, n_inds, f_inds);
...
showDeletedItems = 1; %IF PRINT THE DETAILS OR NOT
...
loopLblsPlot = arrayfun(@(i) [strrep(loopLbls{i},'-n','') ': ' num2str(T(i),'%.2i')],1:length(T),'UniformOutput',0)'
xticks(1:length(distmat));xticklabels(loopLblsPlot(ord))
yticks(1:length(distmat));yticklabels(loopLblsPlot(ord))
% OR PLOT DENDROGRAM
...
wiki.dm_avg = loopDM(sorted_tw2,:);
wiki.noun_clust = T
wiki.nouns = sorted_tw;
wiki.dist_mat = distmat(sorted_tw2,sorted_tw2);
wiki.noun_ord = 1:length(sorted_tw2);


%save('/Users/aidasaglinskas/Desktop/wiki_optimised.mat','wiki')
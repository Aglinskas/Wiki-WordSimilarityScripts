...
load isDonePyMulti isDone
load someDistancePyMulti someDistance
...
for targ_i=1:nTarg
    ...
    for dim_j=1:nPerTry:nDims%:30%length(dimWords)
        ...
        if (sum(isDone(targ_i,dim_j:dim_j+nPerTry-1)==1)==nPerTry)% if SECTION not done
            ...
            title(num2str(dim_j))
            drawnow
            save isDonePyMulti isDone
            
            save someDistancePyMulti someDistance sel_targWords sel_dimWords
        end
    end
 end   

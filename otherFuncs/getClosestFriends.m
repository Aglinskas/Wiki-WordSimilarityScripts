...
A={'red', 'green','blue'};
    A=unique([A {'black'    'blue'    'brown'    'dark'    'green'    'red'    'white'    'yellow'}])
word=[];
% if ~exist('done')
    ...
    for targ_i=1:length(A)
        if ~done(targ_i)
           ...
           [status,result] = system(['python ./google-ngrams-master/getngrams.py ' lower(A{targ_i})  '@*_ADJ -startYear=1960 -smoothing=0 -nosave ']);
                ...
                [status,result] = system(['python ./google-ngrams-master/getngrams.py *_ADJ@ ' lower(A{targ_i}) ' -startYear=1960 -smoothing=0 -nosave ']);
            end
            if length(result)>4
...
ind=ismember(uWord,A); % remove target words from output
word(ind)=[];
uWord(ind)=[];
score=[];

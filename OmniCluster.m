...
featWords = [topVerb topAdjs topNouns];
...
for ii=length(featWords):-1:1  
    if absScore(find(strcmp(featWords{ii},absWord)))<freqThresh
        featWords(ii)=[];
        % marked{end+1} = featWords{ii};
    end
end
%[Y I] = sort(absScore,'descend');absWord(I(1:50))'; 
...
xticks(1:size(dmAll,3))
xticklabels(featWords)
xtickangle(45)
colorbar
disp('done')
...
if corr32
   ...
   drop_nouns = find(sum(wiki.dm_avg,2)<minThresh_targs);
...
[H,clust,OUTPERM] =dendrogram(Z,100,'Labels',wiki.nouns','Colorthresh',1.5);
